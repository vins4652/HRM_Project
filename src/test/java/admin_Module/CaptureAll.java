...
public class CaptureAll {
	...
	public static void main(String[] args) throws IOException {
		...
		// WebElement addCorporateButton =
		// driver.findElement(By.xpath("//button[normalize-space()='Add Corporate']"));
		...
		List<WebElement> corporateNames;
		boolean flag = false;
		JavascriptExecutor jse = (JavascriptExecutor) driver;
		for (;;) {
			corporateNames = driver.findElements(By.cssSelector("tbody tr td:nth-child(2)"));
			try {
				jse.executeScript("window.scrollBy(0,50)");
				for (WebElement corpName : corporateNames) {
					 jse.executeScript("window.scrollBy(0,50)");
					 System.out.println(corpName.getText());
					if (corpName.getText().equals("hdgfjkjs")) {
						flag = true;
